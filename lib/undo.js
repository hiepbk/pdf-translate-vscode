...
(function () {
  ...
  function undoErase() {
    const tool = eraser();
    return !!(tool && tool.undo());
  }

  function redoErase() {
    const tool = eraser();
    return !!(tool && tool.redo());
  }
  ...
  /** Called by erase.js after it takes an annotation off the page. */
  function recordEraseEdit() {
    past.push('erase');
    future.length = 0;
  }
  ...
  function initialise() {
    ...
    window.__pdfTranslateUndo = {
      ...
      recordEraseEdit: recordEraseEdit,
      undo: undo,
      redo: redo,
      depth: function () {
        return past.length;
