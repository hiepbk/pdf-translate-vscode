...
`;
    ...
    mine.forEach(function (highlight) {
      highlight.rects.forEach(function (rect) {
        ...
        if (highlight.written) {
          // Already in the file. Clicking would take the overlay away and
          // leave the annotation behind, so it is left inert rather than
          // offering an action that only half works.
          box.style.pointerEvents = 'none';
          box.style.cursor = 'default';
        } else {
          box.title = 'Click to remove this highlight';
  ...
  /**
   * Tell the host which highlights still need writing.
   *
   * Ones already in the file are held back: they stay on screen, because the
   * loaded document cannot show them without a reload, but sending them again
   * would write a second copy into the PDF on the next save.
   */
  function publish() {
    post({
      type: 'highlights',
      highlights: pending.filter(function (h) {
        return !h.written;
      }),
    });
  }
  ...
  /**
   * The file now holds these highlights as real annotations.
   *
   * The overlay deliberately stays on screen. PDF.js is showing the document
   * it loaded before the save and will not redraw from the file, so clearing
   * the overlay here made the highlights vanish the moment they were saved —
   * which read as the save having failed. Reloading to make PDF.js draw them
   * would fix that at the cost of a visible flash on every save.
   *
   * So the overlay stands in for them until the document is genuinely
   * reloaded, at which point `documentloaded` clears it and PDF.js draws the
   * real annotations instead. Nothing is ever drawn twice.
   *
   * The history is cleared, though: undoing a highlight that is already in the
   * file would take away the overlay and leave the annotation behind.
   */
  function onSaved() {
    pending.forEach(function (highlight) {
      highlight.written = true;
    });
    undoStack.length = 0;
    redoStack.length = 0;
    renderAll();
