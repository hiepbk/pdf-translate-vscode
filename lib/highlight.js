...
.pt-hl {
  ...
  /* Multiply is what makes a highlighter look like a highlighter: the ink
     darkens the page instead of covering it, so the text stays readable. */
  mix-blend-mode: multiply;
}
.pt-hl:hover {
  outline: 1px solid rgba(0, 0, 0, 0.45);
}
#pdfTranslateHighlight.toggled {
  background-color: var(--button-hover-color, rgba(0, 0, 0, 0.25));
}
`;
  ...
  function remove(highlight) {
    const index = pending.indexOf(highlight);
    if (index === -1) {
      return;
    }
    pending.splice(index, 1);
    renderPage(highlight.page);
    publish();
    post({ type: 'edited' });
  }
  ...
  function currentColor() {
    ...
    return input ? input.value : '#ffd400';
  }
  ...
  function onMessage(event) {
    ...
    if (message.type === 'saved') {
      onSaved();
    } else if (message.type === 'toggle-highlight') {
      setActive(!active);
    }
  }
  ...
  function initialise() {
    ...
    button = document.getElementById('pdfTranslateHighlight');
    if (button) {
