...
(function () {
  ...
  window.addEventListener('load', async function () {
    ...
    // ENABLE_STORAGE (3) rather than the default ENABLE_FORMS (2). The worker
    // is only handed the annotation storage in this mode, and without it the
    // `hidden` flag the Erase tool sets is never seen — the annotation would
    // go on being painted into the page canvas after being erased.
    PDFViewerApplicationOptions.set('annotationMode', 3)
    ...
    window.addEventListener('message', async function (event) {
      // The extension host also posts translation results through this channel,
      // so only a genuine reload request may trigger a reload.
